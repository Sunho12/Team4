...
const CONVERSATION_TEMPLATES = [
  ...
  },
  {
    ...
    summary: '청구서 내역에 대한 문의 및 요금 설명 요청. 해외 로밍 데이터 사용으로 인한 추가 요금 발생'
  },
  {
    category: '기기변경',
    sentiment: 'positive' as const,
...
async function seedConversations() {
  console.log('🌱 Conversations 더미 데이터 생성 시작...\n')
  ...
  for (const profile of profiles) {
    console.log(`📞 ${profile.full_name} 처리 중...`)

    // Create 2-4 conversations per customer
    ...
    for (let i = 0; i < convCount; i++) {
      ...
      for (const msg of template.messages) {
        ...
        if (msgError) {
          console.error(`  ❌ Message 생성 실패:`, msgError.message)
        } else {
          totalMessages++
        }
      }
      ...
      if (summaryError) {
        console.error(`  ❌ Summary 생성 실패:`, summaryError.message)
      } else {
        totalSummaries++
      }
    }
  ...
  console.log('\n' + '='.repeat(60))
  console.log('✅ Conversations 더미 데이터 생성 완료!\n')
  console.log('📊 생성 통계:')
